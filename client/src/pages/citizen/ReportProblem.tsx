...
export default function ReportProblem({ full }: { full?: boolean }) {
  ...
  const selectGps = (location: NonNullable<typeof gps>) => {
    setGps(location);
  ...
  const findAddress = async () => {
    if (!gps || addressBusy) return;
    setAddressBusy(true); setAddressErr('');
    try {
      const { location } = await api.post('/geocode/reverse', { lat: gps.lat, lng: gps.lng });
      setAreaName(location.address);
      setLocatedCity(location.city || '');
    } catch (error: any) { setAddressErr(error.message); }
  ...
  const detectGps = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => { selectGps({ lat: pos.coords.latitude, lng: pos.coords.longitude, auto: true }); setGpsBusy(false); },
          ...
          <div className="rounded-lg border border-slate-200 p-3">
            ...
            {gps && (
              <div className="mt-3 space-y-2">
                <p className="text-xs text-slate-500">Find the city/address by sending these coordinates to the configured address provider. Avoid looking up confidential locations.</p>
                <button type="button" className="btn-outline btn-sm" disabled={addressBusy || gpsBusy} onClick={findAddress}>{addressBusy ? 'Finding address…' : 'Find address from GPS'}</button>
                <Field label="City / address / landmark" hint="Check the suggested address and edit it if needed.">
                  <input className="input" value={areaName} disabled={addressBusy} onChange={(e) => setAreaName(e.target.value)} placeholder="Enter an address manually or find it from GPS" />
                </Field>
                {locatedCity && <p className="text-xs text-slate-600">Detected city / locality: <b>{locatedCity}</b></p>}
                {addressErr && <p role="alert" className="text-xs text-rose-600">{addressErr}</p>}
                <p className="text-xs text-slate-400">Address data © <a className="link" href="https://www.openstreetmap.org/copyright" target="_blank" rel="noreferrer">OpenStreetMap contributors</a></p>
              </div>
            )}
            <div className="mt-2 flex flex-wrap items-center gap-1.5">
              ...
              {Object.entries(DEMO_GPS).map(([k, v]) => (
                <button key={k} type="button" disabled={addressBusy || gpsBusy} className="badge bg-slate-100 text-slate-600 hover:bg-brand-50 hover:text-brand-700" onClick={() => selectGps({ ...v, auto: false })}>
                  {v.area.split(',')[0]}
                </button>
              ))}
            </div>
          </div>
          <button className="btn-primary w-full" disabled={addressBusy || gpsBusy} onClick={() => { if (!images.length) { setErr('A photo of the problem is required — it is your evidence.'); return; } setErr(''); setStep(2); }}>Continue <ArrowRight size={15} /></button>
        </div>
      )}
