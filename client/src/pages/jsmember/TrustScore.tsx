...
export default function TrustScore() {
  ...
  useEffect(() => { api.get('/scores/jsmember').then(setInfo); }, []);
  if (!info) return <Spinner />;
  return (
    <div>
      <PageHeader title="Field Trust Score" sub="How reliably you convert assignments into accepted ground truth." />
      <div className="grid gap-6 md:grid-cols-[auto,1fr]">
        <div className="card-p flex flex-col items-center justify-center">
          <ScoreRing score={info.score ?? 0} label="Field Trust Score" />
          <p className="mt-2 text-center text-xs text-slate-500">A high trust score gets you earlier access to new assignments in your area.</p>
        </div>
      ...
      <p className="mt-6 flex items-center justify-center gap-1.5 text-center text-xs text-slate-400"><Trophy size={12} /> JS Members never earn money on JanSetu — trust is the reward.</p>
    </div>
  );
}
