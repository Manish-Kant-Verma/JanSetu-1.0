...
router.post('/:id/confirm', (req: AuthedRequest, res) => {
  ...
  if (c.community.length >= 3 && ['submitted', 'under_review', 'verification_required', 'community_verification'].includes(c.status)) {
...
/** Government approves initial evidence and forwards to the assigned department. */
router.post('/:id/approve', requireRole('government'), (req: AuthedRequest, res) => {
  const db = getDB();
  const c = db.complaints.find((x) => x.id === req.params.id);
  if (!c) return res.status(404).json({ error: 'Complaint not found' });
  if (!canSeeComplaint(req.user!, c)) return res.status(403).json({ error: 'You do not have access to this complaint' });
  if (!['submitted', 'under_review', 'verification_required', 'community_verification', 'verified'].includes(c.status)) {
    return res.status(409).json({ error: 'This complaint has already been forwarded or is no longer awaiting initial approval' });
  }
  if (!c.routedTo) return res.status(400).json({ error: 'Assign an organisation and department before approving this complaint' });
  const note = req.body?.note;
  if (note !== undefined && (typeof note !== 'string' || note.length > 2000)) {
    return res.status(400).json({ error: 'Approval note must be text of at most 2000 characters' });
  }
  pushTimeline(c, 'verified', req.user!.id, req.user!.name, note?.trim() || 'Complaint approved by government after reviewing the submitted evidence');
  pushTimeline(c, 'forwarded_to_government', req.user!.id, req.user!.name, `Approved and forwarded to ${c.routedTo.orgName} — ${c.routedTo.department}. Further action decision pending.`);
  notify(c.citizenId, 'Complaint approved and forwarded', `${c.id} was approved and forwarded to ${c.routedTo.orgName} — ${c.routedTo.department} for further action.`, `/citizen/complaints/${c.id}`);
  notifyOrg(c.routedTo.orgId, `Approved complaint: ${c.id}`, 'Choose direct action, field assessment or contractor work.', `/gov/complaints/${c.id}`);
  audit(req.user, 'COMPLAINT_APPROVED', c.id, { orgId: c.routedTo.orgId, department: c.routedTo.department });
  saveDB();
  res.json({ complaint: c });
});
